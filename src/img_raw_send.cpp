...
int main(int argc, char** argv)
{
    ...
    {
        ImageTCPSend imgSend1(SERVER_IP, 7881);
        ImageTCPSend imgSend2(SERVER_IP, 7882);
        ImageTCPSend imgSend3(SERVER_IP, 7883);
        ImageTCPSend imgSend4(SERVER_IP, 7884);
        ImageTCPSend imgSend5(SERVER_IP, 7885);
        ImageTCPSend imgSend6(SERVER_IP, 7886);
        ImageTCPSend imgSend7(SERVER_IP, 7887);
        ImageTCPSend imgSend8(SERVER_IP, 7888);
        ...
        ros::Subscriber sub_left = nh.subscribe("/camera/color/image_raw", 10, imageCallback);
        ...
        while (ros::ok()) {
            ...
            if (flag == 1) {
                ...
                imgSend1.img = image(Rect(0, 0, w, h));
                imgSend2.img = image(Rect(w, 0, w, h));
                imgSend3.img = image(Rect(halfWidth, 0, w, h));
                imgSend4.img = image(Rect(halfWidth + w, 0, w, h));
           
                imgSend5.img = image(Rect(0, h, w, h));
                imgSend6.img = image(Rect(w, h, w, h));
                imgSend7.img = image(Rect(halfWidth, h, w, h));
                imgSend8.img = image(Rect(halfWidth + w, h, w, h));
